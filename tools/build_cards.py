...
'''
    ...
    <text x="{x}" y="414" font-size="17" fill="{MUTED}">{label}</text>'''
        x += max(tw(num, 46) + 34, tw(label, 17) + 24)   # 숫자·라벨 중 긴 쪽 기준 간격
...
OUTS = {
    "kbrain-ems": hero(
        "kbrain-ems", "#34d399", "교육 운영 시스템", ["모집부터 수료·결과보고까지", "교육 운영 데이터를 한 시스템에서 관리"],
        "운영 중", "행안부 · NIA AI 챔피언 사업", "2026.04 ~ 현재 · 개발 후 운영 병행", "요구분석 · 설계 · 개발 · 운영",
        [("41", "기수"), ("8,180", "지원 건수"), ("1,449", "수료 인원")],
        ["2026년 시스템 도입 후 집계 · 이전 엑셀·구글폼 데이터 이관 없음", "운영 담당자 10명 이상 사용 · 소스는 회사 자산이라 비공개, 사례 문서 공개"]),
    "kbrain-cert": hero(
        "kbrain-cert", "#38bdf8", "인증평가 CBT", ["문항 관리 · 시험 진행 · 실시간 감독 · 결과 내보내기를", "통합한 자체 구축 CBT"],
        "운영 중", "AI 챔피언 인증평가 (행안부 · NIA)", "2026.07 ~ 현재 · 개발 후 운영 병행", "요구분석 · 설계 · 개발 · 운영",
        [("108", "최대 동시 응시"), ("1,466", "누적 응시"), ("1,376", "고유 응시자")],
        ["실시험 운영 집계 (부하테스트 아님) · 동시 응시 설계 기준은 100명", "소스는 회사 자산이라 비공개, 사례 문서 공개"]),
    "jodalfit": hero(
        "jodalfit", "#2dd4bf", "조달핏", ["수주 영업의 공고 탐색을 대신한다", "매일 나라장터를 뒤지는 일을 회사명 입력 한 번으로"],
        "운영 중", "사내 수주 영업 지원 → 공개 서비스 (jodalfit.co.kr)", "2026.05 ~ 현재 · 개발 후 운영 병행", "설계 · 개발 · 데이터 파이프라인",
        [("11,000+", "진행 공고"), ("34,517", "회사 벡터"), ("8종", "나라장터 API")],
        ["자격(면허 · 지역 · 마감) 하드 필터 → pgvector 코사인 → 가중치 랭킹 → LLM 설명",
         "수주이력 중심 설계는 실데이터 매칭 1.1% 확인 후 업종 중심으로 피벗"]),
    "daeasy": card(
        ...
        "케이브레인컴퍼니 · 2026.05 ~ 운영 중 · 요구분석 · 설계 · 개발 · 운영 · 소스 비공개, 사례 문서 공개"),
    "agent-pipeline": card(
